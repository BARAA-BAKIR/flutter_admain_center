...
//  // في _EditStudentViewState
// void _populateControllers(StudentDetails student) {
//   // استخدم setState لتحديث كل شيء دفعة واحدة
//   setState(() {
...
//     // ✅ الإصلاح: تأكد من التعامل مع القيمة الفارغة بشكل صحيح
//     _healthStatusController.text = student.healthStatus ?? '';

//     _gender = student.gender;
//     _socialStatus = student.socialStatus;

//     // ✅ الإصلاح: تعيين القيم المختارة للقوائم المنسدلة
...
class _EditStudentViewState extends State<EditStudentView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocListener<EditStudentBloc, EditStudentState>(
        // ✅ الإصلاح: سنستخدم الـ Listener لتعبئة الحقول
        listener: (context, state) {
          // عندما يتم تحميل البيانات بنجاح
          if (state.status == EditStudentStatus.detailsLoaded &&
              state.student != null) {
            final student = state.student!;
            // نقوم بتعبئة كل شيء هنا مباشرة
            _firstNameController.text = student.firstName;
            _lastNameController.text = student.lastName;
            _fatherNameController.text = student.fatherName ?? '';
            _motherNameController.text = student.motherName ?? '';
            ...
            // نستخدم setState لتحديث القوائم المنسدلة
            setState(() {
              _gender = student.gender;
              _socialStatus = student.socialStatus;
              _selectedHalaqaId = student.halaqan?.id;
            ...
            return BlocBuilder<FilterBloc, FilterState>(
              builder: (context, filterState) {
                ...
                if (editState.student == null) {
                  return Center(
                    child: Text(
                      "فشل تحميل البيانات: ${editState.errorMessage ?? 'خطأ غير معروف'}",
                    ),
                  );
                }

                // الواجهة الآن تعتمد على المتغيرات التي تم تعبئتها في الـ Listener
                return Stepper(
  ...
  List<Step> _buildSteps(BuildContext context, FilterState filterState) {
    // لا تغيير هنا، هذا الكود سليم وسيعمل مع البيانات المعبأة
    return [
      Step(
        title: const Text('المعلومات الشخصية'),
        isActive: _currentStep >= 0,
                    ...
                    filterState.halaqas
                        .map(
                          (halaqa) => DropdownMenuItem<int>(
                            value: halaqa['id'] as int,
                            child: Text(halaqa['name'] as String),
                          ),
                        )
                        .toList(),
                onChanged: (value) => setState(() => _selectedHalaqaId = value),
