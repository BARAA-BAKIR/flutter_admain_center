...
class TeacherManagementBloc
    extends Bloc<TeacherManagementEvent, TeacherManagementState> {
  ...
  TeacherManagementBloc({required SuperAdminRepository repository})
    ...
    on<FetchPrerequisitesForTeacherForm>(_onFetchPrerequisites);
     on<ResetTeacherFormStatus>(_onResetFormStatus);
  }
 void _onResetFormStatus(
    ResetTeacherFormStatus event,
    Emitter<TeacherManagementState> emit,
  ) {
    // أعد الحالة إلى قيمتها الأولية
  ...
  Future<void> _onFetchApproved(
    FetchApprovedTeachers event,
    Emitter<TeacherManagementState> emit,
  ) async {
    emit(
      state.copyWith(
        approvedStatus: TeacherManagementStatus.loading,
        currentPage: 1,
        hasReachedMax: false,
      ),
    );
    final result = await _repository.getApprovedTeachers(
      page: 1,
    ...
    result.fold(
      (failure) => emit(
        state.copyWith(
          approvedStatus: TeacherManagementStatus.failure,
  ...
  Future<void> _onFetchMoreApproved(
    FetchMoreApprovedTeachers event,
    Emitter<TeacherManagementState> emit,
  ) async {
    if (state.hasReachedMax ||
        state.approvedStatus == TeacherManagementStatus.loading)
      return;
    ...
    result.fold(
      ...
      (paginatedData) {
        ...
        emit(
          state.copyWith(
            approvedStatus: TeacherManagementStatus.success,
            approvedTeachers: List.of(state.approvedTeachers)
              ..addAll(newTeachers),
            currentPage: paginatedData['current_page'],
            hasReachedMax: paginatedData['next_page_url'] == null,
          ),
  ...
  Future<void> _onFetchPending(
    ...
    result.fold(
      (failure) => emit(
        state.copyWith(
          pendingStatus: TeacherManagementStatus.failure,
          errorMessage: failure.message,
        ),
      ),
      (users) => emit(
  ...
  Future<void> _onApprove(
    ...
    result.fold(
      (failure) => emit(
        state.copyWith(
          errorMessage: failure.message,
          actionStatus: TeacherActionStatus.failure,
        ),
      ),
      (_) {
        // إزالة الطلب من قائمة الانتظار
        final updatedList = List<PendingUser>.from(state.pendingTeachers)
  ...
  Future<void> _onPromote(
    PromoteTeacher event,
    Emitter<TeacherManagementState> emit,
  ) async {
    final result = await _repository.promoteTeacher(
  ...
  Future<void> _onDeleteTeacher(
    ...
    result.fold(
      ...
      (_) {
        final updatedList = List<Teacher>.from(state.approvedTeachers)
          ..removeWhere((t) => t.id == event.teacherId);
        emit(
          state.copyWith(
