...
class AddEditCenterBloc extends Bloc<AddEditCenterEvent, AddEditCenterState> {
  ...
  Future<void> _onLoadPrerequisites(
    LoadCenterPrerequisites event,
    Emitter<AddEditCenterState> emit,
  ) async {
    emit(state.copyWith(status: FormStatus.loading));

    // جلب قائمة المدراء دائماً
    final managersResult = await _repository.getPotentialManagers(
      editingCenterId: centerId,
    );
    if (managersResult.isLeft()) {
      emit(
        state.copyWith(
    ...
    final managers = managersResult.getOrElse(() => []);

    // إذا كنا في وضع التعديل، نجلب بيانات المركز
    if (event.centerIdToEdit != null) {
      final centerResult = await _repository.getCenterDetails(
        event.centerIdToEdit!,
      );
      centerResult.fold(
        (failure) => emit(
          state.copyWith(
        ...
        (centerData) => emit(
          state.copyWith(
            ...
            initialData: centerData, // ✅ حفظ البيانات الأولية
          ),
        ),
      );
    } else {
      // إذا كنا في وضع الإضافة، نعرض فقط قائمة المدراء
      emit(
        state.copyWith(status: FormStatus.loaded, potentialManagers: managers),
      );
    }
  }

  Future<void> _onSubmitNew(
    SubmitNewCenter event,
    Emitter<AddEditCenterState> emit,
  ) async {
  ...
  Future<void> _onSubmitUpdate(
    SubmitCenterUpdate event,
    Emitter<AddEditCenterState> emit,
  ) async {
    emit(state.copyWith(status: FormStatus.submitting));
    final result = await _repository.updateCenter(
      centerId: centerId!,
      data: event.data,
    );
    result.fold(
