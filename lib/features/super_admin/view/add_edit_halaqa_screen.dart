...
//   final HalaqaModel? halaqa; // لاستقبال الحلقة في وضع التعديل
...
//     return BlocProvider(
//       create:
//           (context) => AddEditHalaqaBloc(
//             repository: context.read<SuperAdminRepository>(),
//           )..add(LoadHalaqaPrerequisites()), // طلب البيانات اللازمة فوراً
//       child: AddEditHalaqaView(halaqa: halaqa), // تمرير الحلقة للـ View
//     );
//   }
// }
...
//   void initState() {
//     super.initState();
//     // إذا كنا في وضع التعديل، نملأ الحقول بالبيانات الحالية
...
//       // إذا كان هناك مركز محدد، نطلب جلب مساجده
//       if (_selectedCenterId != null) {
//         context.read<AddEditHalaqaBloc>().add(
//           CenterSelected(_selectedCenterId!),
//         );
//       }
//     }
//   }
...
//         'type': _selectedTypeId, // غيرنا المفتاح من 'type_id' إلى 'type'
//       };
//       // ✅✅✅ نهاية التعديل ✅✅✅
...
//       appBar: AppBar(
//         title: Text(
//           widget.halaqa == null ? 'إضافة حلقة جديدة' : 'تعديل الحلقة',
//         ),
//       ),
//       body: BlocConsumer<AddEditHalaqaBloc, AddEditHalaqaState>(
//         listener: (context, state) {
//           if (state.status == FormStatus.success) {
//             // عند النجاح، أبلغ القائمة الرئيسية بالتحديث وأغلق الشاشة
...
//           // عرض مؤشر تحميل إذا كانت البيانات الأساسية لم تحمل بعد
...
//                       _selectedMosqueId =
//                           null; // أفرغ اختيار المسجد عند تغيير المركز
//                     });
//                     if (value != null) {
//                       context.read<AddEditHalaqaBloc>().add(
...
//                 ),
//                 const SizedBox(height: 16),
//                 // القائمة المنسدلة للمساجد
//                 DropdownButtonFormField<int>(
//                   value: _selectedMosqueId,
//                   hint: const Text('2. اختر المسجد'),
//                   // يتم بناء القائمة من الحالة التي يتم تحديثها عند اختيار مركز
//                   items:
//                       state.mosques.map((mosque) {
//                         return DropdownMenuItem(
//                           value: mosque['id'] as int,
//                           child: Text(mosque['name'].toString()),
//                         );
//                       }).toList(),
//                   onChanged:
...
//                 ),
//                 const SizedBox(height: 16),
//                 // القائمة المنسدلة لأنواع الحلقات
//                 DropdownButtonFormField<int>(
//                   value: _selectedTypeId,
//                   hint: const Text('3. اختر نوع الحلقة'),
//                   items:
//                       state.halaqaTypes.map((type) {
//                         return DropdownMenuItem(
//                           value: type.id,
...
//                   validator:
//                       (v) => v == null ? 'الرجاء اختيار نوع الحلقة' : null,
//                   decoration: const InputDecoration(
//                     border: OutlineInputBorder(),
            ...
            child: ListView(
              ...
              children: [
                ...
                DropdownButtonFormField<int>(
                  ...
                  validator: (v) => v == null ? 'الرجاء اختيار مسجد' : null,
                  decoration: const InputDecoration(border: OutlineInputBorder(), prefixIcon: Icon(Icons.mosque_rounded)),
                ),
                const SizedBox(height: 16),
                DropdownButtonFormField<int>(
                  value: _selectedTypeId,
                  ...
                  decoration: const InputDecoration(border: OutlineInputBorder(), prefixIcon: Icon(Icons.category_rounded)),
                ),
                const SizedBox(height: 32),
                ElevatedButton.icon(
