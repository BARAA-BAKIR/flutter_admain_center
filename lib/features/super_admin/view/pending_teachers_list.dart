    ...
    return BlocBuilder<TeacherManagementBloc, TeacherManagementState>(
      builder: (context, state) {
        ...
        if (state.pendingStatus == TeacherManagementStatus.failure) {
          return Center(child: Text('فشل تحميل الطلبات: ${state.errorMessage}'));
        }
        ...
        return RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final formattedDate = DateFormat('yyyy-MM-dd').format(DateTime.parse(user.createdAt));
              return Card(
                margin: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                child: ListTile(
                  title: Text(user.name),
                  subtitle: Text('المركز: ${user.centerName ?? 'N/A'}\nتاريخ الطلب: $formattedDate'),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      IconButton(
                        icon: const Icon(Icons.check_circle, color: Colors.green),
                        onPressed: () => context.read<TeacherManagementBloc>().add(ApproveTeacherRequest(user.id)),
                      ),
                      IconButton(
                        icon: const Icon(Icons.cancel, color: Colors.red),
                        onPressed: () => context.read<TeacherManagementBloc>().add(RejectTeacherRequest(user.id)),
                      ),
                    ],
                  ),
                ),
